...
and [<StringEnum>] OrderStringEnum =
    | Desc
    | Asc
...
and [<AllowNullLiteral>] [<Import("LoDashWrapper","Lowdb")>] LoDashWrapper<'LowEntryClass>() =
    ...
    member __.orderBy(iteratee: string, order: string): 'LowEntryClass = jsNative
    member __.orderBy(iteratee: string, order: OrderStringEnum): 'LowEntryClass = jsNative
